...
class SoftAttention(Layer):
    ...
    def build(self, input_shape):
        ...
        self.out_attention_maps_shape = input_shape[0:1] + \
            (self.multiheads,)+input_shape[1:-1]
    ...
    def call(self, x):
        ...
        conv3d = K.reshape(conv3d, shape=(-1, self.multiheads,
                           self.i_shape[1]*self.i_shape[2]))

        softmax_alpha = K.softmax(conv3d, axis=-1)
        softmax_alpha = kl.Reshape(target_shape=(
            self.multiheads, self.i_shape[1], self.i_shape[2]))(softmax_alpha)
        ...
        if self.aggregate_channels == False:
            ...
            u = kl.Reshape(target_shape=(
                self.i_shape[1], self.i_shape[2], u.shape[-1]*u.shape[-2]))(u)
...
def getModel():
    ...
    attention_layer, map2 = SoftAttention(aggregate=True, m=16, concat_with_x=False, ch=int(
        conv.shape[-1]), name='soft_attention')(conv)
    attention_layer = (MaxPooling2D(pool_size=(
        2, 2), padding="same")(attention_layer))
